...
const supabaseKey =
  process.env.SUPABASE_SECRET_KEY || process.env.SUPABASE_ANON_KEY || ""; // allow fallback just in case
let supabase;
...
app.get("/health", (req, res) => {
  res
    .status(200)
    .json({ status: "ok", message: "Node.js backend is running!" });
});
...
io.on("connection", (socket) => {
  ...
  socket.on("start_emergency_session", async () => {
    ...
    if (supabase) {
      ...
      if (error) {
        console.error(
          `❌ [Socket ${socket.id}] Failed to create conversation in DB:`,
          error,
        );
    ...
    openAiWs.on("open", () => {
      ...
      const sessionUpdate = {
        ...
        session: {
          ...
          voice: "sage", // Optional: specify voice
      ...
      if (activeConversationId) {
        ...
        if (fullConversationPcm.length > 0) {
          ...
          uploadToR2(wavBuffer, fileName, "audio/wav")
            ...
            .catch((err) =>
              console.error(
                "Error uploading complete conversation tracking:",
                err,
              ),
            );
        }
